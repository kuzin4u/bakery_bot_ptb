...
async def show_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"🍞 <b>{product[1]}</b>\n\n{product[2]}\n\n💰 <b>Цена:</b> {product[3]:.0f} ₽"
    await query.edit_message_text(text, reply_markup=product_detail_keyboard(product_id, in_cart), parse_mode="HTML")
...
async def add_to_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"🍞 <b>{product[1]}</b>\n\n{product[2]}\n\n💰 <b>Цена:</b> {product[3]:.0f} ₽"
    await query.edit_message_text(text, reply_markup=product_detail_keyboard(product_id, True), parse_mode="HTML")
...
async def remove_from_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if product:
        text = f"🍞 <b>{product[1]}</b>\n\n{product[2]}\n\n💰 <b>Цена:</b> {product[3]:.0f} ₽"
        await query.edit_message_text(text, reply_markup=product_detail_keyboard(product_id, False), parse_mode="HTML")
...
async def show_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cart = user_carts.get(query.from_user.id, [])
    if not cart:
        await query.edit_message_text("🧺 <b>Ваша корзина пуста</b>", reply_markup=main_menu_keyboard(), parse_mode="HTML")
        return
    total = 0
    lines = []
    ...
    await query.edit_message_text(text, reply_markup=cart_keyboard(), parse_mode="HTML")

async def clear_cart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_carts[query.from_user.id] = []
    await query.edit_message_text("🧺 <b>Корзина очищена</b>", reply_markup=main_menu_keyboard(), parse_mode="HTML")
...
async def show_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not orders:
        await query.edit_message_text("📦 <b>У вас пока нет заказов</b>", reply_markup=main_menu_keyboard(), parse_mode="HTML")
        return
...
async def ask_ai_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.edit_message_text(
        "🤖 <b>Задайте ваш вопрос о нашей продукции,</b>\n"
    ...
    await query.edit_message_text(
        "🤖 <b>AI-консультант пока в разработке.</b>\n"
